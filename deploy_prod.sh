...
git commit -m "PROD: DENTRH-REAL-DAM - dentrh plan berie REALNY DAM podla TRHU profilu (SK->OKTE load_okte_dt_for_day, CZ->OTE), nie vzdy cesky OTE. SK profil (Trakany) dostal CZ ceny 162/480 namiesto SK OKTE 205/755 = predikcia/zly trh v dentrh. Ziadny forecast fallback (bez realneho DAM plan nevznikne). Predikcia LEN pre kind=plan (D-1). + PLAN-SOURCE-FIX - realny profil (mode=real) -> plan_source VZDY 'dentrh' + PLAN-SOURCE-DB: pri USE_DB sa plan_source odvodi z mode pri citani z DB (real->dentrh, sim->predicted) bo DB nema stlpec + guard zapisu ak stlpec existuje. + PLAN-KIND-GUARD: _gen_one_plan vynuti kind podla plan_source (dentrh->dentrh, predicted->plan) -> neda sa vyrobit nespravny typ planu. + SOC fixy (A meta-first reader, B2 proj seed, REAL-STATE fallback, uprimny SOC label) + FEASIBILITY-UNIFY KROK 1+2+3 - core/feasibility.py (battery_step+gate+gate_extras = single source fyziky baterie) + core/soc_source.py (kanonicky SOC reader, vdt_state deleguje). KROK 3 DEFAULT ON: advisor VDT clip cez jednu branu gate_extras (SOC^grid^vykon z 1 realneho SOC) namiesto 3 prekryvajucich poistiek (overene DEV VW_3/4). Kill-switch VDT_FEASIBILITY_UNIFIED=0. Golden 5/5 + 122 testov." || echo "(nic nove na commit)"
git push origin dev
