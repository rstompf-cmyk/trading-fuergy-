...
git commit -m "PROD release: FEASIBILITY-UNIFY KROK 1+2+3 - core/feasibility.py (battery_step+gate+gate_extras = single source fyziky baterie) + core/soc_source.py (kanonicky SOC reader, vdt_state deleguje). KROK 3 DEFAULT ON: advisor VDT clip cez jednu branu gate_extras (SOC^grid^vykon z 1 realneho SOC) namiesto 3 prekryvajucich poistiek (overene DEV VW_3/4). Kill-switch VDT_FEASIBILITY_UNIFIED=0. Golden 5/5 + 122 testov." || echo "(nic nove na commit)"
git push origin dev
