        ...
        try:
            ...
            if not tr.empty:
                ...
                try:
                    ...
                    # Bug #623: ODSTRÁNENÝ #613 capacity ledger constraint.
                    # Pôvodný #613 obmedzil _avail_for_dis cez ledger.available() ktorý
                    # vracia (batt_kw_max − Σ rezervácie). Ale D-1 plán SÁM si rezervuje
                    # svoju kapacitu (4000 kW v slote 20:00), takže ledger vrátil len
                    # 6000 − 4000 = 2000 kW voľné. Potom _dis_real = min(plan=4000, ledger=2000)
                    # = 2000 → realita dáva polovičný výkon než D-1 nominácia.
                    # Výsledok: každý slot kde D-1 vybíja > 50% batt_kw → realita
                    # dvojnásobne pod nomináciu → veľká odchýlka → pokuta.
                    # Capacity ledger zostáva v platnosti pre VDT audit gate (#612).
