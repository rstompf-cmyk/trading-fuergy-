...
def fetch_pv_forecast(lat: float, lon: float, kwp: float = 99.0, tilt: float = 30.0,
    ...
    df["kwh"] = df["kw"]
    # FTV-TZ-ALIGN (2026-06-27): open-meteo hodinová radiácia (GTI) pri čase T ≈ priemer [T−1h, T]
    # → produkcia patrí o hodinu skôr (overené: predikcia vrchol 13:00 vs realita 12:00). Realign
    # −1 h cez posun HODNÔT FTV (kw/kwh), NIE času — zachová 24-hod mriežku (žiadny výpadok
    # hraničnej hodiny, čo predtým rozbilo generovanie plánu) a nedotkne sa GTI pre cenový model.
    df["kw"] = df["kw"].shift(-1).fillna(0.0)
    df["kwh"] = df["kw"]
    return df
