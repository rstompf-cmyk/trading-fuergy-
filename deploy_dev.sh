...
git commit -m "dev: manager dashboard v2 (full-width, 1 profil/riadok, cisla+graf, TERAZ marker, navigacia casu) + SOC-leak fix (load_series/paths per-profil) + klik nazov->livesim graf->dentrh + VDT pairs + full-width .container" || echo "(nic nove)"
