...
git add app.py livesim.py vdt_state.py vdt_optimizer.py vdt_live_advisor.py vdt_pair_matcher.py \
        ...
        tests/test_vdt_soc_feasible.py tests/test_holidays_skcz.py tests/test_feasibility_parity.py tests/test_soc_source.py \
...
git commit -m "dev: FEASIBILITY-UNIFY KROK 1+2 - core/feasibility.py (battery_step+gate, single source fyziky; vdt_soc_feasible=wrapper, parity 206) + core/soc_source.py (current_engine_soc+current_soc_pct, kanonicky SOC reader; vdt_state deleguje, cisty presun). Golden 5/5 + 20 testov zelene. Bez zmeny spravania (SOC I/O over na DEV)." || echo "(nic nove)"
